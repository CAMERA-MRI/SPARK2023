...
def filter_split_black_masks(train_list):
  ...
  for idx in train_list:
    mask = np.load(ground_truth_path + train_list[idx].split("/")[5] + "/" +  train_list[idx].split("/")[7])
    if (mask>0).any():
...
def train(args):
    ...
    hprams = {
      "num_epochs": 15,
      "best_loss": 1e10,
      "model_save_path": "/scratch/guest190/models/SAM_noBox",
      "lr":1e-5,
      "weight_decay":0,
  ...
  #-------------------------------------
    ...
    train_loader = DataLoader(train, batch_size= 16, shuffle=False, num_workers=0, sampler=train_sampler)
    val_loader = DataLoader(val, batch_size= 8, shuffle=False, num_workers=0, sampler=val_sampler)
    ...
    model_path = "/scratch/guest190/models/SAM/sam_model_best.pth"
    ...
    wandb.init(project = "SAM Finetune", 
           save_code = True, group= 'Decoder_binary')
