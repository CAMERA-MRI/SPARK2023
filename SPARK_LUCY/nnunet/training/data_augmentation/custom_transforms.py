...
## A custom dataagumentation, unsharp masking, used for the BraTS-africa challenge
## Inorder to increase the sharpness of the image (class name will be modified)
class unsharp_masking_lucy(RandomTransform, IntensityTransform,FourierTransform):
    def __init__(
        self,
        **kwargs,
        ):
    ...
    def apply_transform(self, subject: Subject) -> Subject:
        ...
        for image_name in self.get_images_dict(subject):
            pass
        transform = UnsharpMasking(**self.add_include_exclude(arguments))
        transformed = transform(subject)
        assert isinstance(transformed, Subject)
        return transformed
class UnsharpMasking(FourierTransform,IntensityTransform):
    def __init__(
        self,
        **kwargs,
        ):
    ...
    def add_artifact(
        ...
        G_filtter = GaussianBlur(13, sigma=(0.1, 50.0))
        blurred = G_filtter(p)
        sharpened = p + ((p - blurred) * amount)
        return sharpened