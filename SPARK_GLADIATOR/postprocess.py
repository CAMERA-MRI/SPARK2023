...
def prepare_preditions(e):
    ...
    img = nib.load(f"/scratch/guest188/BraTS_Africa_data/BraTS2023_val/images/{fname}.nii.gz")
    nib.save(
        nib.Nifti1Image(p, img.affine, header=img.header),
        os.path.join("/scratch/guest188/BraTS_Africa_data/final_preds", fname + ".nii.gz"),
    )

os.makedirs("/scratch/guest188/BraTS_Africa_data/final_preds")
preds = sorted(glob(f"/scratch/guest188/BraTS_Africa_data/val_results/predictions*"))
