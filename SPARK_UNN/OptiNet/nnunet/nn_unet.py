...
class NNUnet(pl.LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = {
            # changed to torch optimisers and used Adam fused
            "sgd": torch.optim.SGD(self.parameters(), lr=self.learning_rate, momentum=self.args.momentum),
            "adam": torch.optim.Adam(self.parameters(), lr=self.learning_rate, weight_decay=self.args.weight_decay, fused=True),
