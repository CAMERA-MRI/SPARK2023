...
def train_ae(opt):
    ...
    class Visualizer:
        ...
        def print_current_errors(self, epoch, iter_count, losses, time_per_iter):
            message = '(epoch: {}, iters: {}) '.format(epoch, iter_count)

            for k, v in losses.items():
                message += '{}: {:.3f} '.format(k, v)

                message += '(time: {:.3f}s/iter)'.format(time_per_iter)
                print(message)

        def plot_current_errors(self, losses, total_steps_so_far):
            plt.figure(figsize=(10, 5))
            for k, v in losses.items():
                # Move tensor to CPU before plotting
                v_cpu = v.cpu().detach().numpy()
                plt.plot(total_steps_so_far, v_cpu, label=k)

            plt.xlabel('Total Steps')
            plt.ylabel('Losses')
            plt.title('Current Errors')
            plt.legend()
            plt.grid()
            plt.savefig('current_errors.png')
            plt.close()

        def display_current_results(self, visuals, epoch, total_steps_so_far):
            num_images = len(visuals)
            fig, axes = plt.subplots(1, num_images, figsize=(16, 8))

            for i, (k, v) in enumerate(visuals.items()):
                # Move tensor to CPU before displaying
                v_cpu = v.cpu().detach().numpy()
                if len(v_cpu.shape) == 4:  # If v contains multiple images, plot each one
                    for j in range(v_cpu.shape[0]):
                        ax = axes[i] if num_images > 1 else axes[j]
                        ax.imshow(v_cpu[j, 0], cmap='gray')  # Assuming grayscale images
                        ax.set_title(f"{k} - Epoch: {epoch}, Step: {total_steps_so_far}")
                else:  # If v contains a single image, plot it as usual
                    axes[i].imshow(v_cpu.squeeze(), cmap='gray')  # Assuming grayscale images
                    axes[i].set_title(f"{k} - Epoch: {epoch}, Step: {total_steps_so_far}")

            plt.tight_layout()
            plt.savefig(f"epoch_{epoch}_step_{total_steps_so_far}.png")
            plt.close()   
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoints_dir', default='./ssa_checkpoints/', type=str, help='checkpoint directory')
    parser.add_argument('--checkpoints_dir_pretrained', default='./ssa_checkpoints_pretrained', type=str, help='pretrained models checkpoint directory')
    parser.add_argument('--train_mode', default='end_to_end', type=str, help='training mode')
    parser.add_argument('--max_iterations', default=1) # 1000000
