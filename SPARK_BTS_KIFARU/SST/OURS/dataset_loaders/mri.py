...
class MRIDataset(Dataset):
    ...
    def __init__(self, root_dir, size, base_files, dataset_name, train=True):
        self.anatomies = [f for f in os.listdir(root_dir) if f.endswith('.npy.gz')]
        self.segs      = [f for f in os.listdir(root_dir) if f.endswith('-seg.npy.gz')]
        
        K = Constants(dataset_name)
        # remove the test cases
        for test in K.brain_tests:
            if test in self.anatomies:
                self.anatomies.remove(test + '.npy.gz')
                self.segs.remove(test + '-seg.npy.gz')
...
class MRIDatasetRandom(Dataset):
    ...
    def __init__(self, root_dir, size, dataset_name, train=True):
        ...
        for file_name in self.anatomies[:5]:
            data = load_numpy(os.path.join(root_dir, file_name))
            print(f"Shape of {file_name}: {data.shape}")

        # self.anatomies = [load_numpy(os.path.join(root_dir, f)).transpose(2, 1, 0) for f in self.anatomies]
        self.anatomies = [load_numpy(os.path.join(root_dir, f)) for f in self.anatomies]
...
# def resize_to_target(data, target_shape):
#     zoom_factors = [t / s for s, t in zip(data.shape, target_shape)]
#     resized_data = zoom(data, zoom_factors, order=1)  # Use order=1 for bilinear interpolation
#     return resized_data

# import gzip
# import pickle

# def load_numpy_safe(file_path):
#     try:
#         with gzip.open(file_path, 'rb') as f:
#             data = np.load(f)
#         return data
#     except (EOFError, pickle.UnpicklingError) as e:
#         print(f"Error loading file {file_path}: Compressed file is corrupted. Skipping...")
#         return None
...
#         # Filter out corrupted files and load the remaining ones
#         self.anatomies = [load_numpy_safe(os.path.join(root_dir, f)) for f in self.anatomies if f in self.anatomies]
#         self.anatomies = [a for a in self.anatomies if a is not None]
        
#         for file_name in self.anatomies[:5]:
#             print(f"Shape of {file_name}: {file_name.shape}")
...
class MRIDatasetRandomBaseAtlas(Dataset):
    ...
    def __init__(self, root_dir, base_names, base_path, dataset_name, train=True):
        ...
        self.base_imgs = [[load_numpy(os.path.join(base_path, f + '.npy.gz')).transpose(2, 1, 0),\
                           load_numpy(os.path.join(base_path, f + '-seg.npy.gz')).transpose(2, 1, 0)]\
                           for f in base_names]
