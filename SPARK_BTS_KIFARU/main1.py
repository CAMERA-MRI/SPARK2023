...
# from data_loading.dataset_original
...
def main():
    ...
    if ckpt_path is not None:
        ...
        for key in list(checkpoint_dict.keys()):
            if key not in model_dict or model_dict[key].shape != checkpoint_dict[key].shape:
    ...
    if args.benchmark:
        if args.exec_mode == "train":
            trainer.fit(
                model, train_dataloaders=data_module.train_dataloader())
        else:
            # warmup
            trainer.test(
