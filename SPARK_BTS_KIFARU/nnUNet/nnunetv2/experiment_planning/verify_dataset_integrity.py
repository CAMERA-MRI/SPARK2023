...
def check_cases(image_files: List[str], label_file: str, expected_num_channels: int,
    ...
    print("Shape of images:", shape_image)
    print("Shape of segmentation:", shape_seg)
    if not all([i == j for i, j in zip(shape_image, shape_seg)]):
        ...
        print('Shape images:', shape_image)
        print('Shape seg:', shape_seg)
        print('Image files:', image_files)
        print('Seg file:', label_file)
        ret = False

    # check spacings
    spacing_images = properties_image['spacing']
    spacing_seg = properties_seg['spacing']
    print("Spacing of images:", spacing_images)
    print("Spacing of segmentation:", spacing_seg)
    if not np.allclose(spacing_seg, spacing_images):
        print('Error: Spacing mismatch between segmentation and corresponding images. \nSpacing images: %s. '
              '\nSpacing seg: %s. \nImage files: %s. \nSeg file: %s\n' %
              (shape_image, shape_seg, image_files, label_file))
        print('Spacing images:', spacing_images)
        print('Spacing seg:', spacing_seg)
        print('Image files:', image_files)
        print('Seg file:', label_file)
        ret = False
    ...
    if 'sitk_stuff' in properties_image.keys():
        ...
        if not np.allclose(np.array(origin_image)[:3], np.array(origin_seg)):
        ...
        if not np.allclose(direction_image[:9], direction_seg):
...
# nnUNet_raw = '/scratch/guest189/BraTS2023_data/BraTS_Africa_data/nnUNet_raw_data_base/nnUNet_raw/BraTS2023_africa_train_update/'
if __name__ == "__main__":
    # investigate geometry issues
    example_folder = join(nnUNet_raw, 'Dataset500_BraTS2023')
    num_processes = 6
