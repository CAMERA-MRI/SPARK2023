...
def convert_predicted_logits_to_segmentation_with_correct_shape(predicted_logits: Union[torch.Tensor, np.ndarray],
    ...
    # Print the shape of the segmentation
...
def resample_and_save(predicted: Union[torch.Tensor, np.ndarray], target_shape: List[int], output_file: str,
    ...
    predicted_array_or_file = configuration_manager.resampling_fn_probabilities(predicted,
                                                                                ...
                                                                                current_spacing,
                                                                                target_spacing)
    predicted_shape = np.shape(predicted_array_or_file)  # added!!
    print(predicted_shape)  # Check the shape of the predicted array
    # Compare the shape with the expected shape for segmentation predictions
    # For example, if you expect a 4D shape (C, Z, Y, X) where C is the number of classes, you can do:
    if len(predicted_shape) != 4:
        raise ValueError("The predicted array must have 4 dimensions (C, Z, Y, X) for segmentation predictions.")
