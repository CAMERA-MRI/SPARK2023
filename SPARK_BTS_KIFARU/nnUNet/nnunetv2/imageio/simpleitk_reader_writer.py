...
class SimpleITKIO(BaseReaderWriter):
    ...
    def read_images(self, image_fnames: Union[List[str], Tuple[str, ...]]) -> Tuple[np.ndarray, dict]:
        ...
        for f in image_fnames:
            ...
            if len(npy_image.shape) == 2:
                ...
                raise RuntimeError(
                    "Unexpected number of dimensions: %d in file %s" % (len(npy_image.shape), f))
    ...
    def write_seg(self, seg: np.ndarray, output_fname: str, properties: dict) -> None:
        assert len(
            seg.shape) == 3, 'segmentation must be 3d. If you are exporting a 2d segmentation, please provide it as shape 1,x,y'
