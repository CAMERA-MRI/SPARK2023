...
def create_lists_from_splitted_dataset_folder(folder: str, file_ending: str, identifiers: List[str] = None) -> List[
    ...
    for f in identifiers:
        # print('p calculation!!', re.compile(re.escape(f) + r"-\d\d\d\d" + re.escape(file_ending)))
        p = re.compile(re.escape(f) + r"-\d\d\d\d" + re.escape(file_ending))
        # p = re.compile(re.escape(f) + r"_\d\d\d\d" + re.escape(file_ending))
        # print('P Join!!', [join(folder, i) for i in files if p.fullmatch(i)])
        list_of_lists.append([join(folder, i) for i in files if p.fullmatch(i)])
    print('list of lists!!!', list_of_lists[:5])
...
if __name__ == '__main__':
    get_filenames_of_train_images_and_targets(join(nnUNet_raw, 'Dataset500_BraTS2023')) #print